...
const expectTemplate = (source: WGSLSource, result: TemplateList, expected: {
  ...
  expect({
    parameters: result.parameters.map((p) => source.substring(p.start, p.end + 1)),
    from: result.from,
    to: result.to,
  }).toEqual({
  ...
  expect(source.substring(result.from, result.to + 1)).toEqual(expected.template);
};
...
describe("WGSL - TemplateParser", () => {
  describe("Basic Template Lists", () => {
    it("should parse simple template with one parameter", () => {
      ...
      const result = parser.find(source);
